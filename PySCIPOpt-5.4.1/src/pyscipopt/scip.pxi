...
cdef class Model:
    ...
    def getBranchFeaturesRepresentation(self, candidates, prev_features=None, static_only=False, suppress_warnings=False):
        """
        This function generates 91 branching features for candidate branching variables.
        The features were first used in the following paper:
        @inproceedings{kuang2024rethinking,
            ...
            author={Kuang, Yufei and Wang, Jie and Liu, Haoyang and Zhu, Fangzhou and Li, Xijun and Zeng, Jia and Jianye, HAO and Li, Bin and Wu, Feng},
            booktitle={The Twelfth International Conference on Learning Representations},
            year={2024}
        }

        This function extracts 91 branching features as described in the paper. It is used mainly in the machine 
        learning community for MIP. A user can only call it during the solving process, when there is an LP object.
        This means calling it from some user defined plugin on the Python side.
        An example plugin is a branching rule or an event handler, which is exclusively created to call this function.
        The user must then make certain to return the appropriate SCIP_RESULT (e.g. DIDNOTRUN)
        ...
        ----------
        candidates : list
            List of Variable objects for which to extract features
        prev_features : list of list or None, optional
            The features previously returned by this function
        static_only : bool, optional
            Whether exclusively static features should be generated
        suppress_warnings : bool, optional
            Whether warnings should be suppressed
        ...
        features : list of list
            The extracted features for each candidate variable
        feature_map : dict
            The mapping of feature names to indices
        root_info : dict
            Information about the root node, to be used in subsequent calls
        """
        ...
        cdef SCIP_VARTYPE vtype
        cdef SCIP_Real sim, prod
        cdef int i, j, k, col_i, cand_i, neighbor_index, neighbor_column_index
        cdef int cdeg_max, cdeg_min, cdeg, nb_neighbors, neighbor_ncolumns
        cdef int pcoefs_count, ncoefs_count, active_count, nbranchings_up, nbranchings_down
        cdef int row_index, ncands, nrows
        ...
        cdef SCIP_Real coef, lb, ub, solval, activity, lhs, rhs, cst
        cdef SCIP_Real abs_coef, constraint_sum, neighbor_coef
        cdef SCIP_VAR* var
        ...
        cdef float cdeg_mean, cdeg_var, root_cdeg_mean, root_cdeg_min, root_cdeg_max
        cdef float pcoefs_var, pcoefs_mean, pcoefs_min, pcoefs_max
        cdef float ncoefs_var, ncoefs_mean, ncoefs_min, ncoefs_max
        cdef float ps_up, ps_down, ps_sum
        cdef float prhs_ratio_max, prhs_ratio_min, nrhs_ratio_max, nrhs_ratio_min
        cdef float ota_pp_max, ota_pp_min, ota_pn_max, ota_pn_min
        cdef float ota_np_max, ota_np_min, ota_nn_max, ota_nn_min
        cdef float value, pratio, nratio, pos_coef_sum, neg_coef_sum
        ...
        cdef float acons_nb1, acons_nb2, acons_nb3, acons_nb4, coef_norm
        ...
        # Define the number of features and feature mappings based on static_only
        if static_only:
            n_features = 22  # Only the static features (variable type, obj coeffs, root info)
            feature_map = {
                # Variable type (4 features)
                "binary": 0, "integer": 1, "continuous": 2, "implicit_integer": 3,
                # Objective function coefficients (3 features)
                "obj_coef": 4, "obj_coef_pos": 5, "obj_coef_neg": 6,
                # Number of constraints (1 feature)
                "nnzrs": 7,
                # Statistics for constraint degrees at root (4 features)
                "root_cdeg_mean": 8, "root_cdeg_var": 9, "root_cdeg_min": 10, "root_cdeg_max": 11,
                # Statistics for constraint coefficients at root (10 features)
                "root_pcoefs_count": 12, "root_pcoefs_mean": 13, "root_pcoefs_var": 14, "root_pcoefs_min": 15, "root_pcoefs_max": 16,
                "root_ncoefs_count": 17, "root_ncoefs_mean": 18, "root_ncoefs_var": 19, "root_ncoefs_min": 20, "root_ncoefs_max": 21,
            }
        else:
            n_features = 91
            feature_map = {
                # Variable type (4 features)
                "binary": 0, "integer": 1, "continuous": 2, "implicit_integer": 3,
                # Normalized coefficient (1 feature)
                "norm_coef": 4,
                # Specified bounds (2 features)
                "has_lb": 5, "has_ub": 6,
                # Lower/upper bound (2 features)
                "sol_at_lb": 7, "sol_at_ub": 8,
                # Solution fractionality (1 feature)
                "sol_frac": 9,
                # Categorical (4 features)
                "at_lb": 10, "at_ub": 11, "between_bounds": 12, "at_zero": 13,
                # Reduced cost (1 feature)
                "red_cost": 14,
                # Age (1 feature)
                "age": 15,
                # Solution value (1 feature)
                "sol_val": 16,
                # Incumbent value (1 feature)
                "inc_val": 17,
                # Average incumbent value (1 feature)
                "avg_inc_val": 18,
                # Statistics for active constraint coefficients (24 features)
                "acons_sum1": 19, "acons_mean1": 20, "acons_var1": 21, "acons_max1": 22, "acons_min1": 23,
                "acons_sum2": 24, "acons_mean2": 25, "acons_var2": 26, "acons_max2": 27, "acons_min2": 28,
                "acons_sum3": 29, "acons_mean3": 30, "acons_var3": 31, "acons_max3": 32, "acons_min3": 33,
                "acons_sum4": 34, "acons_mean4": 35, "acons_var4": 36, "acons_max4": 37, "acons_min4": 38,
                "acons_nb1": 39, "acons_nb2": 40, "acons_nb3": 41, "acons_nb4": 42,
                # Statistics for constraint degrees (7 features)
                "cdeg_mean": 43, "cdeg_var": 44, "cdeg_min": 45, "cdeg_max": 46,
                "cdeg_mean_ratio": 47, "cdeg_min_ratio": 48, "cdeg_max_ratio": 49,
                # Objective function coefficients (3 features)
                "obj_coef": 50, "obj_coef_pos": 51, "obj_coef_neg": 52,
                # Infeasibility statistics (4 features)
                "nb_up_infeas": 53, "nb_down_infeas": 54, "frac_up_infeas": 55, "frac_down_infeas": 56,
                # Number of constraints (1 feature)
                "nnzrs": 57,
                # Min/max for ratios of constraint coefficients to RHS (4 features)
                "prhs_ratio_max": 58, "prhs_ratio_min": 59, "nrhs_ratio_max": 60, "nrhs_ratio_min": 61,
                # Min/max for one-to-all coefficient ratios (8 features)
                "ota_pp_max": 62, "ota_pp_min": 63, "ota_pn_max": 64, "ota_pn_min": 65,
                "ota_np_max": 66, "ota_np_min": 67, "ota_nn_max": 68, "ota_nn_min": 69,
                # Pseudocosts (5 features)
                "ps_up": 70, "ps_down": 71, "ps_ratio": 72, "ps_sum": 73, "ps_product": 74,
                # Statistics for constraint degrees at root (4 features)
                "root_cdeg_mean": 75, "root_cdeg_var": 76, "root_cdeg_min": 77, "root_cdeg_max": 78,
                # Statistics for constraint coefficients at root (10 features)
                "root_pcoefs_count": 79, "root_pcoefs_mean": 80, "root_pcoefs_var": 81, "root_pcoefs_min": 82, "root_pcoefs_max": 83,
                "root_ncoefs_count": 84, "root_ncoefs_mean": 85, "root_ncoefs_var": 86, "root_ncoefs_min": 87, "root_ncoefs_max": 88,
                # Slack and ceil distances (2 features)
                "slack": 89, "ceil_dist": 90,
            }

        # Initialize the features
        ncands = len(candidates)
        if prev_features is None:
            features = [[0 for i in range(n_features)] for j in range(ncands)]
        else:
            assert len(prev_features) > 0, "Previous features is empty"
            features = prev_features
            if len(prev_features) != ncands:
                if not suppress_warnings:
                    raise Warning(f"The number of candidates has changed. Previous data being ignored")
                    if len(prev_features[0]) != n_features:
                        raise Warning(f"Dimension mismatch in provided previous features and new features:"
                                    f"{len(prev_features[0])} != {n_features}")
                else:
                    features = [[0 for i in range(n_features)] for j in range(ncands)]
                    prev_features = None
        ...
        # Extract static features and root information
        if SCIPgetNNodes(scip) == 1:
            ...
            # Collect root information for all candidate variables
        ...
        for cand_i in range(ncands):
            ...
            vtype = SCIPvarGetType(var)
            if vtype == SCIP_VARTYPE_BINARY:
                features[cand_i][feature_map["binary"]] = 1
            elif vtype == SCIP_VARTYPE_INTEGER:
                features[cand_i][feature_map["integer"]] = 1
            elif vtype == SCIP_VARTYPE_CONTINUOUS:
                features[cand_i][feature_map["continuous"]] = 1
            elif vtype == SCIP_VARTYPE_IMPLINT:
                features[cand_i][feature_map["implicit_integer"]] = 1

            # Objective function coefficients (3 features)
            features[cand_i][feature_map["obj_coef"]] = SCIPcolGetObj(col)
            features[cand_i][feature_map["obj_coef_pos"]] = max(features[cand_i][feature_map["obj_coef"]], 0)
            features[cand_i][feature_map["obj_coef_neg"]] = min(features[cand_i][feature_map["obj_coef"]], 0)

            # Number of constraints (1 feature)
            features[cand_i][feature_map["nnzrs"]] = SCIPcolGetNNonz(col)

            # Statistics for constraint degrees at root (4 features)
            if 'col' in root_info and col_i in root_info['col']['root_cdeg_mean']:
                features[cand_i][feature_map["root_cdeg_mean"]] = root_info['col']['root_cdeg_mean'][col_i]
                features[cand_i][feature_map["root_cdeg_var"]] = root_info['col']['root_cdeg_var'][col_i]
                features[cand_i][feature_map["root_cdeg_min"]] = root_info['col']['root_cdeg_min'][col_i]
                features[cand_i][feature_map["root_cdeg_max"]] = root_info['col']['root_cdeg_max'][col_i]

                # Statistics for constraint coefficients at root (10 features)
                features[cand_i][feature_map["root_pcoefs_count"]] = root_info['col']['root_pcoefs_count'][col_i]
                features[cand_i][feature_map["root_pcoefs_mean"]] = root_info['col']['root_pcoefs_mean'][col_i]
                features[cand_i][feature_map["root_pcoefs_var"]] = root_info['col']['root_pcoefs_var'][col_i]
                features[cand_i][feature_map["root_pcoefs_min"]] = root_info['col']['root_pcoefs_min'][col_i]
                features[cand_i][feature_map["root_pcoefs_max"]] = root_info['col']['root_pcoefs_max'][col_i]
                features[cand_i][feature_map["root_ncoefs_count"]] = root_info['col']['root_ncoefs_count'][col_i]
                features[cand_i][feature_map["root_ncoefs_mean"]] = root_info['col']['root_ncoefs_mean'][col_i]
                features[cand_i][feature_map["root_ncoefs_var"]] = root_info['col']['root_ncoefs_var'][col_i]
                features[cand_i][feature_map["root_ncoefs_min"]] = root_info['col']['root_ncoefs_min'][col_i]
                features[cand_i][feature_map["root_ncoefs_max"]] = root_info['col']['root_ncoefs_max'][col_i]

            # If only static features are requested, continue to the next candidate
            if static_only:
                continue
            ...
            # Specified bounds (2 features)
            if not SCIPisInfinity(scip, abs(lb)):
                features[cand_i][feature_map["has_lb"]] = 1
            if not SCIPisInfinity(scip, abs(ub)):
                features[cand_i][feature_map["has_ub"]] = 1
            
            # Lower/upper bound (2 features)
            features[cand_i][feature_map["sol_at_lb"]] = int(SCIPisEQ(scip, solval, lb))
            features[cand_i][feature_map["sol_at_ub"]] = int(SCIPisEQ(scip, solval, ub))
            
            # Solution fractionality (1 feature)
            features[cand_i][feature_map["sol_frac"]] = SCIPfeasFrac(scip, solval)
            
            # Categorical (4 features)
            features[cand_i][feature_map["at_lb"]] = int(SCIPisEQ(scip, solval, lb))
            features[cand_i][feature_map["at_ub"]] = int(SCIPisEQ(scip, solval, ub))
            features[cand_i][feature_map["between_bounds"]] = int(not SCIPisEQ(scip, solval, lb) and not SCIPisEQ(scip, solval, ub))
            features[cand_i][feature_map["at_zero"]] = int(SCIPisZero(scip, solval))
            
            # Reduced cost (1 feature)
            features[cand_i][feature_map["red_cost"]] = SCIPgetColRedcost(scip, col)
            
            # Age (1 feature)
            features[cand_i][feature_map["age"]] = SCIPcolGetAge(col)
            
            # Solution value (1 feature)
            features[cand_i][feature_map["sol_val"]] = solval
            
            # Incumbent value (1 feature) and Average incumbent value (1 feature)
            sol = SCIPgetBestSol(scip)
            if sol is NULL:
                features[cand_i][feature_map["inc_val"]] = float('nan')
                features[cand_i][feature_map["avg_inc_val"]] = float('nan')
            else:
                features[cand_i][feature_map["inc_val"]] = SCIPgetSolVal(scip, sol, var)
                features[cand_i][feature_map["avg_inc_val"]] = SCIPvarGetAvgSol(var)
            
            # Slack and ceil distances (2 features)
            features[cand_i][feature_map["slack"]] = min(features[cand_i][feature_map["sol_frac"]], 1 - features[cand_i][feature_map["sol_frac"]])
            features[cand_i][feature_map["ceil_dist"]] = 1 - features[cand_i][feature_map["sol_frac"]]
            
            # Pseudocosts (5 features)
            ps_up = SCIPgetVarPseudocost(scip, var, SCIP_BRANCHDIR_UPWARDS)
            ps_down = SCIPgetVarPseudocost(scip, var, SCIP_BRANCHDIR_DOWNWARDS)
            ps_sum = ps_up + ps_down
            features[cand_i][feature_map["ps_up"]] = ps_up
            features[cand_i][feature_map["ps_down"]] = ps_down
            features[cand_i][feature_map["ps_sum"]] = ps_sum
            features[cand_i][feature_map["ps_ratio"]] = 0 if ps_sum == 0 else ps_up / ps_sum
            features[cand_i][feature_map["ps_product"]] = ps_up * ps_down
            
            # Infeasibility statistics (4 features)
            features[cand_i][feature_map["nb_up_infeas"]] = SCIPvarGetCutoffSum(var, SCIP_BRANCHDIR_UPWARDS)
            features[cand_i][feature_map["nb_down_infeas"]] = SCIPvarGetCutoffSum(var, SCIP_BRANCHDIR_DOWNWARDS)
            nbranchings_up = SCIPvarGetNBranchings(var, SCIP_BRANCHDIR_UPWARDS)
            nbranchings_down = SCIPvarGetNBranchings(var, SCIP_BRANCHDIR_DOWNWARDS)
            features[cand_i][feature_map["frac_up_infeas"]] = 0 if nbranchings_up == 0 else features[cand_i][feature_map["nb_up_infeas"]] / nbranchings_up
            features[cand_i][feature_map["frac_down_infeas"]] = 0 if nbranchings_down == 0 else features[cand_i][feature_map["nb_down_infeas"]] / nbranchings_down
            ...
            if nb_neighbors > 0:
                ...
                for neighbor_index in range(nb_neighbors):
                    ...
                    cdeg_var += (cdeg - cdeg_mean)**2
                cdeg_var /= nb_neighbors
            features[cand_i][feature_map["cdeg_mean"]] = cdeg_mean
            features[cand_i][feature_map["cdeg_var"]] = cdeg_var
            features[cand_i][feature_map["cdeg_min"]] = cdeg_min
            features[cand_i][feature_map["cdeg_max"]] = cdeg_max
            
            # Ratios for constraint degrees
            if 'col' in root_info and col_i in root_info['col']['root_cdeg_mean']:
                root_cdeg_mean = root_info['col']['root_cdeg_mean'][col_i]
                root_cdeg_min = root_info['col']['root_cdeg_min'][col_i]
                root_cdeg_max = root_info['col']['root_cdeg_max'][col_i]
                features[cand_i][feature_map["cdeg_mean_ratio"]] = 0 if cdeg_mean == 0 else cdeg_mean / (root_cdeg_mean + cdeg_mean)
                features[cand_i][feature_map["cdeg_min_ratio"]] = 0 if cdeg_min == 0 else cdeg_min / (root_cdeg_min + cdeg_min)
                features[cand_i][feature_map["cdeg_max_ratio"]] = 0 if cdeg_max == 0 else cdeg_max / (root_cdeg_max + cdeg_max)
            
            # Min/max for ratios of constraint coefficients to RHS (4 features) and Min/max for one-to-all coefficient ratios (8 features)
            nonzero_coefs_raw = SCIPcolGetVals(col)
            ...
            for neighbor_index in range(nb_neighbors):
                ...
                if not SCIPisInfinity(scip, abs(lhs)):
                    value = 0 if coef == 0 else coef / (abs(coef) + abs(lhs))
                    if -lhs >= 0:
                        prhs_ratio_max = max(prhs_ratio_max, value)
                        prhs_ratio_min = min(prhs_ratio_min, value)
                    else:
                ...
                if coef > 0:
                    pratio = 0 if pos_coef_sum == 0 else coef / pos_coef_sum
                    nratio = 0 if (coef - neg_coef_sum) == 0 else coef / (coef - neg_coef_sum)
                ...
                if coef < 0:
                    pratio = 0 if (coef - pos_coef_sum) == 0 else coef / (coef - pos_coef_sum)
                    nratio = 0 if neg_coef_sum == 0 else coef / neg_coef_sum
            ...
            features[cand_i][feature_map["prhs_ratio_max"]] = prhs_ratio_max
            features[cand_i][feature_map["prhs_ratio_min"]] = prhs_ratio_min
            features[cand_i][feature_map["nrhs_ratio_max"]] = nrhs_ratio_max
            features[cand_i][feature_map["nrhs_ratio_min"]] = nrhs_ratio_min
            features[cand_i][feature_map["ota_pp_max"]] = ota_pp_max
            features[cand_i][feature_map["ota_pp_min"]] = ota_pp_min
            features[cand_i][feature_map["ota_pn_max"]] = ota_pn_max
            features[cand_i][feature_map["ota_pn_min"]] = ota_pn_min
            features[cand_i][feature_map["ota_np_max"]] = ota_np_max
            features[cand_i][feature_map["ota_np_min"]] = ota_np_min
            features[cand_i][feature_map["ota_nn_max"]] = ota_nn_max
            features[cand_i][feature_map["ota_nn_min"]] = ota_nn_min
            
            # Statistics for active constraint coefficients (24 features)
            # First, identify active constraints and compute weights
            nrows = SCIPgetNLPRows(scip)
            rows = SCIPgetLPRows(scip)
            
            # Initialize arrays for active constraint weights
            act_cons_w1 = np.zeros(shape=(nrows, ), dtype=np.float32)  # Unit weight
            act_cons_w2 = np.zeros(shape=(nrows, ), dtype=np.float32)  # Inverse of sum of all coefficients
            act_cons_w3 = np.zeros(shape=(nrows, ), dtype=np.float32)  # Inverse of sum of candidate coefficients
            act_cons_w4 = np.zeros(shape=(nrows, ), dtype=np.float32)  # Dual cost
            ...
            for row_index in range(nrows):
                ...
                # Check if constraint is active (binding with equality)
                if SCIPisEQ(scip, activity, rhs) or SCIPisEQ(scip, activity, lhs):
                    ...
                    # Weight no. 1: Unit weight
                    act_cons_w1[row_index] = 1
                    
                    # Weight no. 2: Inverse of sum of coefficients of all variables
                    constraint_sum = 0
                    for neighbor_column_index in range(neighbor_ncolumns):
                        constraint_sum += abs(neighbor_columns_values[neighbor_column_index])
                    act_cons_w2[row_index] = 1 if constraint_sum == 0 else 1 / constraint_sum
                    
                    # Weight no. 3: Inverse of sum of coefficients of candidate variables
                    constraint_sum = 0
                    for neighbor_column_index in range(neighbor_ncolumns):
                        neighbor_var = SCIPcolGetVar(neighbor_columns[neighbor_column_index])
                        for j in range(ncands):
                            c_var = (<Variable>candidates[j]).scip_var
                            if SCIPvarGetIndex(c_var) == SCIPvarGetIndex(neighbor_var):
                                constraint_sum += abs(neighbor_columns_values[neighbor_column_index])
                                break
                    act_cons_w3[row_index] = 1 if constraint_sum == 0 else 1 / constraint_sum
                    
                    # Weight no. 4: Dual cost of constraint
                    act_cons_w4[row_index] = abs(SCIProwGetDualsol(row))
            
            # Now compute active constraint features for the current variable
            acons_sum1 = 0
            acons_mean1 = 0
            acons_var1 = 0
            acons_max1 = 0
            ...
            for neighbor_index in range(nb_neighbors):
                ...
                if SCIPisEQ(scip, activity, rhs) or SCIPisEQ(scip, activity, lhs):
                    ...
                    abs_coef = abs(nonzero_coefs_raw[neighbor_index])
            ...
            # Calculate means and variances for active constraints
            if active_count > 0:
            ...
            # Assign active constraint features
            features[cand_i][feature_map["acons_sum1"]] = acons_sum1
            features[cand_i][feature_map["acons_mean1"]] = acons_mean1
            features[cand_i][feature_map["acons_var1"]] = acons_var1
            features[cand_i][feature_map["acons_max1"]] = acons_max1
            features[cand_i][feature_map["acons_min1"]] = acons_min1
            
            features[cand_i][feature_map["acons_sum2"]] = acons_sum2
            features[cand_i][feature_map["acons_mean2"]] = acons_mean2
            features[cand_i][feature_map["acons_var2"]] = acons_var2
            features[cand_i][feature_map["acons_max2"]] = acons_max2
            features[cand_i][feature_map["acons_min2"]] = acons_min2
            
            features[cand_i][feature_map["acons_sum3"]] = acons_sum3
            features[cand_i][feature_map["acons_mean3"]] = acons_mean3
            features[cand_i][feature_map["acons_var3"]] = acons_var3
            features[cand_i][feature_map["acons_max3"]] = acons_max3
            features[cand_i][feature_map["acons_min3"]] = acons_min3
            
            features[cand_i][feature_map["acons_sum4"]] = acons_sum4
            features[cand_i][feature_map["acons_mean4"]] = acons_mean4
            features[cand_i][feature_map["acons_var4"]] = acons_var4
            features[cand_i][feature_map["acons_max4"]] = acons_max4
            features[cand_i][feature_map["acons_min4"]] = acons_min4
            
            features[cand_i][feature_map["acons_nb1"]] = acons_nb1
            features[cand_i][feature_map["acons_nb2"]] = acons_nb2
            features[cand_i][feature_map["acons_nb3"]] = acons_nb3
            features[cand_i][feature_map["acons_nb4"]] = acons_nb4
            
            # Normalized coefficient (1 feature)
            # This is calculated as the objective coefficient divided by the norm of the variable's coefficients
            coef_norm = 0.0
            for neighbor_index in range(nb_neighbors):
                coef = float(nonzero_coefs_raw[neighbor_index])
                coef_norm += float(coef * coef)
            coef_norm = float(coef_norm)
            coef_norm = coef_norm ** 0.5 if coef_norm > 0 else 1
            features[cand_i][feature_map["norm_coef"]] = features[cand_i][feature_map["obj_coef"]] / coef_norm

        return features, feature_map, root_info

    def getCutSelectionFeatures(self, cuts=None, prev_var_features=None, prev_cons_features=None, 
                            prev_cut_features=None, static_only=False, suppress_warnings=False):
        """
        This function generates features for variables, constraints, and cuts for cutting plane selection
        as described in Zhang et al., "Learning to Select Cutting Planes in Mixed Integer Linear Programming Solving."
        
        Parameters
        ----------
        cuts : list of Row or None, optional
            The list of cut rows to extract features for
        prev_var_features : list of list or None, optional
            The list of variable features previously returned by this function
        prev_cons_features : list of list or None, optional
            The list of constraint features previously returned by this function
        prev_cut_features : list of list or None, optional
            The list of cut features previously returned by this function
        static_only : bool, optional
            Whether exclusively static features should be generated
        suppress_warnings : bool, optional
            Whether warnings should be suppressed
        ...
        Returns
        -------
        var_features : list of list
            Features for each variable
        cons_features : list of list
            Features for each constraint
        cut_features : list of list or None
            Features for each cut
        dict
            The feature mappings for the variables, constraints, and cuts
        """
        from math import sqrt
        
        cdef SCIP* scip = self._scip
        cdef SCIP_VARTYPE vtype
        cdef int col_i, i, j, k
        ...
        # Generate variable features
        cdef SCIP_COL** cols = SCIPgetLPCols(scip)
        cdef int ncols = SCIPgetNLPCols(scip)

        # Define variable feature map
        if static_only:
            n_var_features = 5
            var_feature_map = {"binary": 0, "integer": 1, "implint": 2, "continuous": 3, "norm_coef": 4}
        else:
            n_var_features = 17
            var_feature_map = {"binary": 0, "integer": 1, "implint": 2, "continuous": 3, "norm_coef": 4,
                            "has_lb": 5, "has_ub": 6, "norm_redcost": 7, "solval": 8, "solfrac": 9,
                            "sol_is_at_lb": 10, "sol_is_at_ub": 11, "norm_age": 12,
                            "basis_lower": 13, "basis_basic": 14, "basis_upper": 15, "basis_zero": 16}

        # Initialize or validate variable features
        if prev_var_features is None:
            var_features = [[0 for i in range(n_var_features)] for j in range(ncols)]
        else:
            assert len(prev_var_features) > 0, "Previous variable features is empty"
            var_features = prev_var_features
            if len(prev_var_features) != ncols:
                if not suppress_warnings:
                    raise Warning(f"The number of variables has changed. Previous variable data being ignored")
                else:
                    var_features = [[0 for i in range(n_var_features)] for j in range(ncols)]
                    prev_var_features = None
            if len(prev_var_features[0]) != n_var_features:
                raise Warning(f"Dimension mismatch in provided previous features and new features:"
                            f"{len(prev_var_features[0])} != {n_var_features}")
        ...
        for i in range(ncols):
            ...
            # Collect static features if needed
            if prev_var_features is None:
                # Variable types
                vtype = SCIPvarGetType(var)
                if vtype == SCIP_VARTYPE_BINARY:
                    var_features[col_i][var_feature_map["binary"]] = 1
                elif vtype == SCIP_VARTYPE_INTEGER:
                    var_features[col_i][var_feature_map["integer"]] = 1
                elif vtype == SCIP_VARTYPE_CONTINUOUS:
                    var_features[col_i][var_feature_map["continuous"]] = 1
                elif vtype == SCIP_VARTYPE_IMPLINT:
                    var_features[col_i][var_feature_map["implint"]] = 1
                
                # Objective coefficient normalized by objective norm
                if obj_norm > 0:
                    var_features[col_i][var_feature_map["norm_coef"]] = SCIPcolGetObj(cols[i]) / obj_norm
                else:
                    var_features[col_i][var_feature_map["norm_coef"]] = SCIPcolGetObj(cols[i])

            # Collect dynamic features
            if not static_only:
                # Lower bound
                if not SCIPisInfinity(scip, abs(lb)):
                    var_features[col_i][var_feature_map["has_lb"]] = 1

                # Upper bound
                if not SCIPisInfinity(scip, abs(ub)):
                    var_features[col_i][var_feature_map["has_ub"]] = 1

                # Reduced cost normalized by objective norm
                if obj_norm > 0:
                    var_features[col_i][var_feature_map["norm_redcost"]] = SCIPgetColRedcost(scip, cols[i]) / obj_norm
                else:
                    var_features[col_i][var_feature_map["norm_redcost"]] = SCIPgetColRedcost(scip, cols[i])

                # LP solution value
                var_features[col_i][var_feature_map["solval"]] = solval
                var_features[col_i][var_feature_map["solfrac"]] = SCIPfeasFrac(scip, solval)
                var_features[col_i][var_feature_map["sol_is_at_lb"]] = int(SCIPisEQ(scip, solval, lb))
                var_features[col_i][var_feature_map["sol_is_at_ub"]] = int(SCIPisEQ(scip, solval, ub))

                # Age normalized by number of solved LPs
                if nlps > 0:
                    var_features[col_i][var_feature_map["norm_age"]] = SCIPcolGetAge(cols[i]) / nlps
                else:
                    var_features[col_i][var_feature_map["norm_age"]] = SCIPcolGetAge(cols[i])

                # Basis status
                basis_status = SCIPcolGetBasisStatus(cols[i])
                if basis_status == SCIP_BASESTAT_LOWER:
                    var_features[col_i][var_feature_map["basis_lower"]] = 1
                elif basis_status == SCIP_BASESTAT_BASIC:
                    var_features[col_i][var_feature_map["basis_basic"]] = 1
                elif basis_status == SCIP_BASESTAT_UPPER:
                    var_features[col_i][var_feature_map["basis_upper"]] = 1
                elif basis_status == SCIP_BASESTAT_ZERO:
                    var_features[col_i][var_feature_map["basis_zero"]] = 1

        # Generate constraint features
        cdef SCIP_ROW** rows = SCIPgetLPRows(scip)
        cdef int nrows = SCIPgetNLPRows(scip)

        # Define constraint feature map
        if static_only:
            n_cons_features = 6
            cons_feature_map = {"norm_nnzrs": 0, "norm_bias_lhs": 1, "norm_bias_rhs": 2, "norm": 3, 
                            "is_integral": 4, "is_local": 5}
        else:
            n_cons_features = 15
            cons_feature_map = {"norm_nnzrs": 0, "norm_bias_lhs": 1, "norm_bias_rhs": 2, "norm": 3, 
                            "is_integral": 4, "is_local": 5, "row_is_at_lhs": 6, "row_is_at_rhs": 7, 
                            "norm_dualsol": 8, "norm_age": 9, "norm_intcols": 10, "is_removable": 11,
                            "basis_lower": 12, "basis_basic": 13, "basis_upper": 14, "basis_zero": 15}

        # Initialize or validate constraint features
        if prev_cons_features is None:
            cons_features = [[0 for i in range(n_cons_features)] for j in range(nrows)]
        else:
            assert len(prev_cons_features) > 0, "Previous constraint features is empty"
            cons_features = prev_cons_features
            if len(prev_cons_features) != nrows:
                if not suppress_warnings:
                    raise Warning(f"The number of constraints has changed. Previous constraint data being ignored")
                else:
                    cons_features = [[0 for i in range(n_cons_features)] for j in range(nrows)]
                    prev_cons_features = None
            if len(prev_cons_features[0]) != n_cons_features:
                raise Warning(f"Dimension mismatch in provided previous features and new features:"
                            f"{len(prev_cons_features[0])} != {n_cons_features}")

        cdef SCIP_Real lhs, rhs, cst, row_norm, row_activity
        cdef int n_total_cols
        ...
        for i in range(nrows):
            ...
            rhs = SCIProwGetRhs(rows[i])
            cst = SCIProwGetConstant(rows[i])
            row_activity = SCIPgetRowLPActivity(scip, rows[i])
            row_norm = SCIProwGetNorm(rows[i])

            if prev_cons_features is None:
                # Number of nonzeros
                cons_features[i][cons_feature_map["norm_nnzrs"]] = SCIProwGetNLPNonz(rows[i])
                
                # Norm of the row
                cons_features[i][cons_feature_map["norm"]] = row_norm
                
                # Unshifted side normalized by row norm
                if not SCIPisInfinity(scip, abs(lhs)) and row_norm > 0:
                    cons_features[i][cons_feature_map["norm_bias_lhs"]] = (lhs - cst) / row_norm
                
                if not SCIPisInfinity(scip, abs(rhs)) and row_norm > 0:
                    cons_features[i][cons_feature_map["norm_bias_rhs"]] = (rhs - cst) / row_norm
                
                # Is integral
                cons_features[i][cons_feature_map["is_integral"]] = SCIProwIsIntegral(rows[i])
                
                # Is local
                cons_features[i][cons_feature_map["is_local"]] = SCIProwIsLocal(rows[i])

            if not static_only:
                # Is row at LHS or RHS
                cons_features[i][cons_feature_map["row_is_at_lhs"]] = int(SCIPisEQ(scip, row_activity, lhs))
                cons_features[i][cons_feature_map["row_is_at_rhs"]] = int(SCIPisEQ(scip, row_activity, rhs))
                
                # Dual solution normalized by row norm and objective norm
                if row_norm > 0 and obj_norm > 0:
                    cons_features[i][cons_feature_map["norm_dualsol"]] = SCIProwGetDualsol(rows[i]) / (row_norm * obj_norm)
                else:
                    cons_features[i][cons_feature_map["norm_dualsol"]] = SCIProwGetDualsol(rows[i])
                
                # Age normalized by number of solved LPs
                if nlps > 0:
                    cons_features[i][cons_feature_map["norm_age"]] = SCIProwGetAge(rows[i]) / nlps
                else:
                    cons_features[i][cons_feature_map["norm_age"]] = SCIProwGetAge(rows[i])
                
                # Number of integral columns in the row normalized
                n_total_cols = SCIProwGetNNonz(rows[i])
                if n_total_cols > 0:
                    cons_features[i][cons_feature_map["norm_intcols"]] = SCIPgetRowNumIntCols(scip, rows[i]) / n_total_cols
                else:
                    cons_features[i][cons_feature_map["norm_intcols"]] = 0
                
                # Is removable
                cons_features[i][cons_feature_map["is_removable"]] = SCIProwIsRemovable(rows[i])
                
                # Basis status
                basis_status = SCIProwGetBasisStatus(rows[i])
                if basis_status == SCIP_BASESTAT_LOWER:
                    cons_features[i][cons_feature_map["basis_lower"]] = 1
                elif basis_status == SCIP_BASESTAT_BASIC:
                    cons_features[i][cons_feature_map["basis_basic"]] = 1
                elif basis_status == SCIP_BASESTAT_UPPER:
                    cons_features[i][cons_feature_map["basis_upper"]] = 1
                elif basis_status == SCIP_BASESTAT_ZERO:
                    cons_features[i][cons_feature_map["basis_zero"]] = 1

        # If no cuts are provided, return only variable and constraint features
        if cuts is None or len(cuts) == 0:
            return (var_features, cons_features, None,
                    {"var": var_feature_map, "cons": cons_feature_map, "cut": None})

        # Generate cut features
        n_cuts = len(cuts)
        
        # Define cut feature map
        if static_only:
            n_cut_features = 5
            cut_feature_map = {"support": 0, "integral_support": 1, "norm": 2, "norm_bias_lhs": 3, "norm_bias_rhs": 4}
        else:
            n_cut_features = 13
            cut_feature_map = {"support": 0, "integral_support": 1, "norm": 2, "norm_bias_lhs": 3, "norm_bias_rhs": 4,
                            "efficacy": 5, "parallelism": 6, "normalized_violation": 7,
                            "coef_mean": 8, "coef_max": 9, "coef_min": 10, "coef_std": 11,
                            "obj_parallelism": 12}

        # Initialize or validate cut features
        if prev_cut_features is None:
            cut_features = [[0 for i in range(n_cut_features)] for j in range(n_cuts)]
        else:
            assert len(prev_cut_features) > 0, "Previous cut features is empty"
            cut_features = prev_cut_features
            if len(prev_cut_features) != n_cuts:
                if not suppress_warnings:
                    raise Warning(f"The number of cuts has changed. Previous cut data being ignored")
                else:
                    cut_features = [[0 for i in range(n_cut_features)] for j in range(n_cuts)]
                    prev_cut_features = None
            if len(prev_cut_features[0]) != n_cut_features:
                raise Warning(f"Dimension mismatch in provided previous features and new features:"
                            f"{len(prev_cut_features[0])} != {n_cut_features}")

        # Extract cut features
        for i in range(n_cuts):
            # Using Row object's methods to get properties
            cut = cuts[i]
            ...
            cut_nnzrs = cut.getNCols()
            
            # Calculate support and integral support
            if cut_nnzrs > 0 and ncols > 0:
                cut_features[i][cut_feature_map["support"]] = cut_nnzrs / ncols
                
                # Calculate integral support
                n_int_nonzeros = 0
                valid_nnzrs = 0
                
                for k in range(cut_nnzrs):
                    # Use Column API methods instead of direct C calls
                    col = cut_cols[k]
                    col_idx = col.getLPPos()
                    if col_idx != -1:  # Only count variables in the LP
                        valid_nnzrs += 1
                        var_obj = col.getVar()
                        vtype = var_obj.vtype()
                        if vtype == "BINARY" or vtype == "INTEGER" or vtype == "IMPLINT":
                            n_int_nonzeros += 1
                
                cut_features[i][cut_feature_map["integral_support"]] = n_int_nonzeros / valid_nnzrs if valid_nnzrs > 0 else 0
            
            # Norm
            cut_features[i][cut_feature_map["norm"]] = cut_norm
            
            # Unshifted sides normalized by cut norm
            if not SCIPisInfinity(scip, abs(cut_lhs)) and cut_norm > 0:
                cut_features[i][cut_feature_map["norm_bias_lhs"]] = (cut_lhs - cut_cst) / cut_norm
            
            if not SCIPisInfinity(scip, abs(cut_rhs)) and cut_norm > 0:
                cut_features[i][cut_feature_map["norm_bias_rhs"]] = (cut_rhs - cut_cst) / cut_norm
            
            if not static_only:
                # Use the existing methods from the pyscipopt API
                # Efficacy
                cut_features[i][cut_feature_map["efficacy"]] = self.getCutEfficacy(cut)
                
                # Parallelism with objective
                parallelism = self.getRowObjParallelism(cut)
                cut_features[i][cut_feature_map["parallelism"]] = parallelism
                cut_features[i][cut_feature_map["obj_parallelism"]] = parallelism
                
                # Calculate normalized violation
                # We can approximate this using getLPActivity method
                activity = self.getRowLPActivity(cut)
                
                if not SCIPisInfinity(scip, abs(cut_rhs)) and abs(cut_rhs - cut_cst) > 1e-6:
                    violation = max(0, (activity - cut_rhs) / abs(cut_rhs - cut_cst))
                    cut_features[i][cut_feature_map["normalized_violation"]] = violation
                
                if not SCIPisInfinity(scip, abs(cut_lhs)) and abs(cut_lhs - cut_cst) > 1e-6:
                    violation = max(0, (cut_lhs - activity) / abs(cut_lhs - cut_cst))
                    if "normalized_violation" in cut_feature_map:
                        cut_features[i][cut_feature_map["normalized_violation"]] = max(
                            cut_features[i][cut_feature_map["normalized_violation"]], violation)
                    else:
                        cut_features[i][cut_feature_map["normalized_violation"]] = violation
                
                # Statistics of cut coefficients - only include variables in the LP
                valid_coeffs = []
                for k in range(cut_nnzrs):
                    col = cut_cols[k]
                    col_idx = col.getLPPos()
                    if col_idx != -1:
                        valid_coeffs.append(cut_vals[k])
                
                if valid_coeffs:
                    cut_features[i][cut_feature_map["coef_mean"]] = sum(valid_coeffs) / len(valid_coeffs)
                    cut_features[i][cut_feature_map["coef_max"]] = max(valid_coeffs)
                    cut_features[i][cut_feature_map["coef_min"]] = min(valid_coeffs)
                    
                    # Standard deviation
                    if len(valid_coeffs) > 1:
                        mean = sum(valid_coeffs) / len(valid_coeffs)
                        variance = sum((x - mean) ** 2 for x in valid_coeffs) / len(valid_coeffs)
                        cut_features[i][cut_feature_map["coef_std"]] = sqrt(variance)
                    else:
                        cut_features[i][cut_feature_map["coef_std"]] = 0
                else:
                    cut_features[i][cut_feature_map["coef_mean"]] = 0
                    cut_features[i][cut_feature_map["coef_max"]] = 0
                    cut_features[i][cut_feature_map["coef_min"]] = 0
                    cut_features[i][cut_feature_map["coef_std"]] = 0

        return (var_features, cons_features, cut_features,
                {"var": var_feature_map, "cons": cons_feature_map, "cut": cut_feature_map})
    ...
    def getVanillafullstrongData(self):
        ...
        PY_SCIP_CALL(SCIPgetVanillafullstrongData(self._scip, &cands, &candscores, &ncands, &npriocands, &bestcand))
    ...
    # def executeBranchRuleOld(self, str name, allowaddcons):
    ...
    def executeBranchRule(self, str name, allowaddcons):
        """
        Execute a branching rule by name.
        
        Notes:
        - For 'relpscost', uses SCIP's native SCIPexecRelpscostBranching function
        - For 'vanillafullstrong', simulates execution using getVanillafullstrongData
        - Other rules require SCIP core modifications for direct execution
        
        Parameters:
        -----------
        name : str
            Name of the branching rule to execute
        allowaddcons : bool
            Whether to allow adding constraints
            
        Returns:
        --------
        result : SCIP_RESULT
            Result of the branching rule execution
        """
        cdef SCIP_BRANCHRULE* branchrule
        cdef SCIP_RESULT result 
        cdef SCIP_VAR** c_branchcands = NULL
        cdef SCIP_Real* c_branchcandssol = NULL
        cdef SCIP_Real* c_branchcandsfrac = NULL
        cdef int nbranchcands = 0
        cdef int npriobranchcands = 0
        cdef int i
        cdef SCIP_Real solval

        cdef char* param_name
        cdef int old_rule_prio = 0
        ...
        branchrule = SCIPfindBranchrule(self._scip, name.encode("UTF-8"))
        if branchrule == NULL:
            # For vanillafullstrong, try to include it
            if name == "vanillafullstrong":
                PY_SCIP_CALL(SCIPincludeBranchruleVanillafullstrong(self._scip))
                branchrule = SCIPfindBranchrule(self._scip, "vanillafullstrong")
                if branchrule == NULL:
                    print("Error: Failed to include vanillafullstrong branching rule!")
                    return PY_SCIP_RESULT.DIDNOTFIND
            # For pscost, try to include it
            elif name == "pscost":
                PY_SCIP_CALL(SCIPincludeBranchrulePscost(self._scip))
                branchrule = SCIPfindBranchrule(self._scip, "pscost")
                if branchrule == NULL:
                    print("Error: Failed to include pscost branching rule!")
                    return PY_SCIP_RESULT.DIDNOTFIND
            else:
                print(f"Error: Branching rule '{name}' not found!")
                return PY_SCIP_RESULT.DIDNOTFIND
        
        # Handle different branching rules
        if name == "relpscost":
            # Get branch candidates from SCIP
            candidate_vars, nbranchcands, npriobranchcands = self.getPseudoBranchCands()
            if nbranchcands == 0:
                return PY_SCIP_RESULT.DIDNOTRUN
            
            # Allocate arrays for candidates
            c_branchcands = <SCIP_VAR**> malloc(nbranchcands * sizeof(SCIP_VAR*))
            c_branchcandssol = <SCIP_Real*> malloc(nbranchcands * sizeof(SCIP_Real))
            c_branchcandsfrac = <SCIP_Real*> malloc(nbranchcands * sizeof(SCIP_Real))
            
            if c_branchcands == NULL or c_branchcandssol == NULL or c_branchcandsfrac == NULL:
                if c_branchcands != NULL:
                    free(c_branchcands)
                if c_branchcandssol != NULL:
                    free(c_branchcandssol)
                if c_branchcandsfrac != NULL:
                    free(c_branchcandsfrac)
                raise MemoryError()
            
            # Fill the arrays
            for i in range(nbranchcands):
                var = candidate_vars[i]
                c_branchcands[i] = (<Variable>var).scip_var
                # Get LP solution value for this variable
                solval = SCIPvarGetLPSol((<Variable>var).scip_var)
                c_branchcandssol[i] = solval
                # Calculate the fractional part
                c_branchcandsfrac[i] = SCIPfeasFrac(self._scip, solval)
            
            # Execute relpscost branching
            PY_SCIP_CALL(SCIPexecRelpscostBranching(self._scip, c_branchcands, c_branchcandssol, 
                                                c_branchcandsfrac, nbranchcands, True, &result))
        ...
        elif name == "vanillafullstrong" or name == "pscost":
            if name == "vanillafullstrong":
                param_name = "branching/vanillafullstrong/priority"
                # For vanillafullstrong, need to set additional parameter (do branching)
                PY_SCIP_CALL(SCIPsetBoolParam(self._scip, "branching/vanillafullstrong/donotbranch", False))
            elif name == "pscost":
                param_name = "branching/pscost/priority"
            else:
                # For other rules, no direct execution API is available
                print(f"Note: Direct execution of '{name}' is not supported in standard SCIP.")
                return PY_SCIP_RESULT.DIDNOTRUN
        
        
            # Save current priorities of all branching rules to restore later
            PY_SCIP_CALL(SCIPgetIntParam(self._scip, param_name, &old_rule_prio))
            PY_SCIP_CALL(SCIPsetIntParam(self._scip, param_name, 1000000))
                        
            # Execute branching directly using SCIP's branching mechanism
            PY_SCIP_CALL(SCIPbranchLP(self._scip, &result))
            
            # Restore original priority
            PY_SCIP_CALL(SCIPsetIntParam(self._scip, param_name, old_rule_prio))
        ...
        # Convert the C result to the Python equivalent
        if result == SCIP_BRANCHED:
            return PY_SCIP_RESULT.BRANCHED
        elif result == SCIP_CUTOFF:
            return PY_SCIP_RESULT.CUTOFF
        elif result == SCIP_REDUCEDDOM:
            return PY_SCIP_RESULT.REDUCEDDOM
        elif result == SCIP_DIDNOTFIND:
            return PY_SCIP_RESULT.DIDNOTFIND
        elif result == SCIP_DIDNOTRUN:
            return PY_SCIP_RESULT.DIDNOTRUN
        else:
            return PY_SCIP_RESULT.DIDNOTRUN
