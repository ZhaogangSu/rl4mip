...
def get_best_expression_from_json_file(instance_type):
...
class NodeDataset(torch.utils.data.Dataset):
    ...
    # def load(self):
    #     os.makedirs(self.processed_dir, exist_ok=True)
    #     info_dict_path = osp.join(self.processed_dir, "info_dict_ns.pt")
    ...
    def load(self):
        ...
        if osp.exists(info_dict_path) and False:
            # 修改代码
            info_dict = torch.load(info_dict_path, weights_only=False)
            file_names = info_dict["file_names"]
            processed_files = info_dict["processed_files"]
            ...
            raw_file_names = os.listdir(self.raw_dir)
            random.shuffle(raw_file_names)
            if len(raw_file_names) < self.data_num:
                print(colored(f'WARNING: The total number of data sets is {len(raw_file_names)}, which is less than the expected {self.data_num}', 'red'))
            ...
            processed_files = []
            info_dict.update(processed_files=processed_files, file_names=file_names)
            # print("info_dict\n", len(info_dict["processed_files"]))
        if self.data_num > len(processed_files):
            file_name_list = []
            for file_name in file_names[len(processed_files):self.data_num]:
                file_name_list.append(file_name)
            processed_file = self.process_sample(file_name_list)
            processed_files.append(processed_file)
            self._data_list = processed_files
