...
class Expression(nn.Module):
    ...
    def forward(self, inputs, device):
        # 修改代码，把字符串里的 'const.DEVICE' 换掉
        final_device = 'cpu'
        if not isinstance(device, str):
            if device.type == 'cuda':
                if device.index != None and device.index >= 0:
                    final_device = device.type + ":" + str(device.index)
        
        final_expression = self.expression.replace('consts.DEVICE', f'\'{final_device}\'')
        res = F.tanh(eval(final_expression))
        return res
...
class ExpressionFromStr(nn.Module):
    ...
    def forward(self, inputs):     
        # print(inputs)
        res = F.tanh(eval(self.expression))
        return res
...
class EnsemBleExpression(nn.Module):
    ...
    def forward(self, x, device,train_mode=True):
        with set_train_mode(self, train_mode):
            futures = [torch.jit.fork(model, x, device=device) for model in self.models]
