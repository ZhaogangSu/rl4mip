...
class RegressionTree:
    ...
    def _get_split_mse(col: ndarray, label: ndarray, split: float) -> Node:
        ...
        left_mse = ((label_left - avg_left) ** 2).sum() / len(label_left)
        right_mse = ((label_right - avg_right) ** 2).sum() / len(label_right)

        # Create nodes to store result.
        node = Node(split=split, mse=mse)
        node.left = Node(avg_left, mse = left_mse)
        node.right = Node(avg_right, mse = right_mse)
    ...
    def calc_one(self, row: ndarray) -> float:
        """Auxiliary function of predict.
    ...
    def calc(self, data: ndarray) -> ndarray:
        ...
        return np.apply_along_axis(self.calc_one, 1, data)