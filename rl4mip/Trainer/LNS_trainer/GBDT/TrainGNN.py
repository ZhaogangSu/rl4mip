...
def process(policy, data_loader, device, optimizer=None):
    ...
    with torch.set_grad_enabled(optimizer is not None):
        for batch in data_loader:
            ...
            #print(logits)
            loss = loss_funcA(logits, batch.assignment1.to(torch.int64)) 
            #loss += loss_funcB(logits, batch.assignment2.to(torch.int64))
...
def trainGNN(
    ...
    pair_train = os.path.join(data_dir, 'instances', method, 'pair_data', problem, 'train')
    print(pair_train)
    sample_files = [str(path) for path in Path(os.path.join(data_dir, 'instances', method, 'pair_data', problem, 'train')).glob("*.pkl")]
    print(sample_files)
    train_files = sample_files
    valid_files = [str(path) for path in Path(os.path.join(data_dir, 'instances', method, 'pair_data', problem, 'valid')).glob("*.pkl")]
    ...
    model_save_path = f'{model_dir}/{method}/{problem}/GNN_{problem}' + '.pkl'
