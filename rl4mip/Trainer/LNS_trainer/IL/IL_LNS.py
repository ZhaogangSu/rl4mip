...
from rl4mip.Trainer.LNS_model.IL_model.gnn_policy import GNNPolicy
from rl4mip.Trainer.LNS_model.IL_model.losses import LogScoreLoss, LinearScoreLoss 
from rl4mip.Trainer.LNS_model.IL_model.gcn_pre import GraphDataset_pre, GNNPolicy_pre
from rl4mip.DataCollector.LNS_data.IL_data.utils import augment_variable_features_with_dynamic_ones
from rl4mip.DataCollector.LNS_data.IL_data.utils import pad_tensor, multi_hot_encoding
from rl4mip.DataCollector.LNS_data.IL_data import bipartite_graph_loader as bgl
...
class ILTrainer_Pre():
    def __init__(self, model_dir, data_dir, method, problem, device, batch_size = 4):
        ...
        self.model_save_path = os.path.join(model_dir, method, problem, f"Pre_model_{problem}/")
        
        os.makedirs(self.model_save_path, exist_ok=True)

        train_dir_sol = os.path.join(self.data_dir, "sol_data", method, problem, "train")
        train_dir_bg = os.path.join(self.data_dir, "bg_data", method, problem, "train")

        valid_dir_sol = os.path.join(self.data_dir, "sol_data", method, problem, "valid")
        valid_dir_bg = os.path.join(self.data_dir, "bg_data", method, problem, "valid")

        train_names = os.listdir(train_dir_bg)
        self.train_files = [ (os.path.join(train_dir_bg, name), os.path.join(train_dir_sol, name).replace('bg','sol')) for name in train_names]

        valid_names = os.listdir(valid_dir_bg)
        self.valid_files = [ (os.path.join(valid_dir_bg, name), os.path.join(valid_dir_sol, name).replace('bg','sol')) for name in valid_names]

    def EnergyWeightNorm(self, task):
        if task=="setcover":
            return -1000
        elif task=="mvc":
            return -1000
    ...
    def pre_train(self, predict, data_loader, optimizer, weight_norm, device):
        ...
        with torch.set_grad_enabled(optimizer is not None): # 优化器存在则计算梯度
            for step, batch in enumerate(data_loader): # 训练数据
                batch = batch.to(device)
                # get target solutions in list format
                solInd = batch.nsols # 解空间的个数
                target_sols = [] # 定义目标解
                target_vals = [] # 定义目标变量
                solEndInd = 0
                valEndInd = 0
                ...
                for i in range(solInd.shape[0]): # for in batch
                    ...
                    target_sols.append(sols) # 目标解 目标变量
                ...
                for ind, (sols,vals) in enumerate(zip(target_sols, target_vals)):
                    ...
                    # print("sum_loss", sum_loss.sum())
                    
                    sample_loss = sum_loss*weight[:,None]

                    # print("sample_loss", sample_loss.sum())

                    loss += sample_loss.sum()

                    # print("ind loss", ind, loss)
        ...
        # print(n_samples_processed, mean_loss)
    ...
    def pre_model(self, lr_init, epoches):


        train_data = GraphDataset_pre(self.train_files) # 转换为图数据
        train_loader = torch_geometric.loader.DataLoader(train_data, batch_size=self.batch_size, shuffle=True, num_workers=0)
        valid_data = GraphDataset_pre(self.valid_files)
        valid_loader = torch_geometric.loader.DataLoader(valid_data, batch_size=self.batch_size, shuffle=False, num_workers=0)

        PredictModel = GNNPolicy_pre().to(self.device) # 图卷积输出变量特征
        optimizer = torch.optim.Adam(PredictModel.parameters(), lr=lr_init)
        weight_norm = self.EnergyWeightNorm(self.problem)
        ...
        for epoch in range(epoches):
            ...
            begin=time.time()
            train_loss = self.pre_train(PredictModel, train_loader, optimizer, weight_norm, self.device)
            # writer.add_scalar('train_loss', train_loss, epoch)
            print(f"Epoch {epoch} Train loss: {train_loss:0.3f}")
            valid_loss = self.pre_train(PredictModel, valid_loader, None, weight_norm, self.device)
            # writer.add_scalar('valid_loss', valid_loss, epoch)
            print(f"Epoch {epoch} Valid loss: {valid_loss:0.3f}")
            if valid_loss<best_val_loss:
                best_val_loss = valid_loss
                torch.save(PredictModel.state_dict(),self.model_save_path + 'model_best.pth')
            torch.save(PredictModel.state_dict(), self.model_save_path + 'model_last.pth')
