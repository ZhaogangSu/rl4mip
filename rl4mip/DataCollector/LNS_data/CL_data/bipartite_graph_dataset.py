...
class BipartiteGraphDataset(torch_geometric.data.Dataset):
    ...
    def get(self, index):
        ...
        required_modules = [
            'DataCollector',
            'DataCollector.lns_CL',
            'DataCollector.lns_data',
            'DataCollector.lns_data.CL_data'
        ]
        
        for mod in required_modules:
            if mod not in sys.modules:
                sys.modules[mod] = ModuleType(mod)
                print(f"Dynamically created dummy module: {mod}")

        # 3. 安全加载pickle数据
        try:
            graph = pickle.loads(raw)
        except Exception as e:
            print(f"Failed to load pickle data at index {index}: {str(e)}")
            # 尝试更彻底的修复
            pickle.Unpickler.dispatch = {
                k: v for k, v in pickle.Unpickler.dispatch.items() 
                if not k.startswith('DataCollector')
            }
            graph = pickle.loads(raw)
        # graph = pickle.loads(raw)
        #from IPython import embed; embed()
