...
def generate_mvc(graph, filename):
    """
    Generate a Minimum Vertex Cover instance in CPLEX LP format from a Graph object.

    Parameters
    ----------
    graph : Graph
        The graph from which to build the vertex cover problem.
    filename : str
        Path to the file to save.
    """
    with open(filename, 'w') as lp_file:
        ...
        lp_file.write("OBJ:" + "".join([f" + 1 x{node+1}" for node in range(graph.number_of_nodes)]) + "\n")

        # Constraints: For each edge, at least one endpoint must be in the vertex cover
        lp_file.write("\nsubject to\n")
        for count, (u, v) in enumerate(graph.edges):
            lp_file.write(f"C{count+1}: x{u+1} + x{v+1} >= 1\n")

        # Binary decision variables
        lp_file.write("\nbinary\n")
        lp_file.write(" ".join([f"x{node+1}" for node in range(graph.number_of_nodes)]) + "\n")
...
def generate_maxcut(graph, filename):
    """
    ...
    with open(filename, 'w') as lp_file:
        ...
        for current_edge in graph.edges:
            start = current_edge[0]
            end = current_edge[1]
            idx = "u" + str(start) + "_" + str(end)
            lp_file.write("+" + str(random.random()) + " " + idx + '\n')
            variable_list.append(idx)

            start_var = "v" + str(start)
            end_var = "v" + str(end)
        ...
        for i in range(len(start_list)):
            ...
            var = variable_list[i]
            start_var = start_list[i]
            end_var = end_list[i]

            count = 2 * i

            lp_file.write(f"R{count}:" + "" + var + " + " + start_var + " + " + end_var + " <= 2\n")
            lp_file.write(f"R{count + 1}:" + "" + var + " - " + start_var + " - " + end_var + " <= 0\n")
        ...
        for i in range(len(start_list)):
            ...
            if start_var not in used_list:
                lp_file.write(start_var + "\n")
                used_list.append(start_var)
            
            if end_var not in used_list:
                lp_file.write(end_var + "\n")
                used_list.append(end_var)