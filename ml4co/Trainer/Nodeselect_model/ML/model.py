...
class RankNet(torch.nn.Module):
    ...
    def __init__(self):
        ...
        # self.linear1 = torch.nn.Linear(20, 50).double()
        # self.activation = torch.nn.LeakyReLU()
        # self.linear2 = torch.nn.Linear(50, 1).double()
    ...
    def forward(self, n0, n1, device):
        # 修改代码
        # print(n0.dtype)  # 查看 n0 的数据类型
        # print(n1.dtype)  # 查看 n1 的数据类型
        n0 = n0.to(torch.float32)
        n1 = n1.to(torch.float32)

        # 修改代码
        n0 = torch.where(n0 == float('inf'), torch.tensor(1e5, device=device), n0)
        n0 = torch.where(n0 == -float('inf'), torch.tensor(-1e5, device=device), n0)
        n1 = torch.where(n1 == float('inf'), torch.tensor(1e5, device=device), n1)
        n1 = torch.where(n1 == -float('inf'), torch.tensor(-1e5, device=device), n1)

        s0, s1 = self.forward_node(n0), self.forward_node(n1)
        # print("n0, n1:", n0, ", ", n1)
        # print("-s0, s1:", -s0, ", ", s1)
        # print("torch.sigmoid(-s0 + s1):", torch.sigmoid(-s0 + s1))
