...
def make_obs(instance, seed):
    ...
    m.setRealParam('limits/time', 720) # 10s collect features
...
def get_feat(filepath):
    ...
    obj = m.getObjective()
    obj_cons = [0]*(nvars + 2)    
    indices_spr = [[], []] # 目标函数与变量 约束与变量
    values_spr = [] # 目标函数中变量出现的个数 约束中变量出现次数的个数
    obj_node = [0, 0, 0, 0]

    for e in obj:
        vnm = e.vartuple[0].name # 目标函数
        c_o = obj[e] # 目标函数系数
        c_o_idx = v_map[vnm] # 目标函数变量索引
        obj_cons[c_o_idx] = c_o
        v_nodes[c_o_idx][0] = c_o # 变量特征的第一个维度为在目标函数中的系数
        obj_node[0] += c_o # 目标函数系数和
        obj_node[1] += 1 # 目标函数中变量个数
    ...
    cons_map = sorted(cons_map, key=lambda x: [x[1], str(x[0])]) # 按照约束长短进行排序（第一因素），约束名称排序（第二因素）。
    #print(cons_map)
    cons = [x[0] for x in cons_map] # 约束
    # print(cons)
    lcons = ncons
    c_nodes = [] # 约束特征
    ...
    for cind, c in enumerate(cons):
        coeff = m.getValsLinear(c)
        # coeff[mvars[ncons+cind-1]] = 1 # 添加松弛变量部分
        # print(coeff)
        ...
        for k in coeff:
            #print(k)
            #print(coeff[k])
            v_indx = v_map[k] # 字典中约束变量索引位置
            if coeff[k] != 0:
                indices_spr[0].append(cind) # 第几个约束
                indices_spr[1].append(v_indx) # 第几个变量
                values_spr.append(1)
            v_nodes[v_indx][2] += 1 # 约束中变量的个数 约束和变量的度
            v_nodes[v_indx][1] += coeff[k]/lcons # 变量在所有约束中的平均系数
            v_nodes[v_indx][3] = max(v_nodes[v_indx][3], coeff[k]) # 约束中变量系数的最大值，初始化为0
            v_nodes[v_indx][4] = min(v_nodes[v_indx][4], coeff[k]) # 约束中变量系数的最小值，初始化为一个无穷大的数
            summation += coeff[k] # 约束中所有系数的和
        llc = max(len(coeff), 1) # 约束长度
        c_nodes.append([summation/llc, llc, rhs, sense]) # 约束特征

    #c_nodes.append(obj_node) # 添加目标特征行
    ...
    # print(indices_spr, values_spr)
    A = torch.sparse_coo_tensor(indices_spr, values_spr, (ncons, nvars))
        # c_nodes.append()
    dense_matrix = A.to_dense()

    """
    防止异常值出现,限制变量范围,归一化
    """
    clip_max = [20000, 1, torch.max(v_nodes, 0)[0][2].item()] # torch.max 计算张量在维度0上的最大值的函数, 每个特征维度上的最大值

    clip_min = [0, -1, 0]

    v_nodes[:,0] = torch.clamp(v_nodes[:,0], clip_min[0], clip_max[0]) # 防止不合理数据出现

    maxs = torch.max(v_nodes, 0)[0] # 按一维取最大
    # print(maxs) 
    mins = torch.min(v_nodes, 0)[0] # 按一维取最小
    ...
    v_nodes = v_nodes - mins # 减去最小值
    v_nodes = v_nodes / diff # 特征值最大最小归一化
    v_nodes = torch.clamp(v_nodes, 1e-5, 1) # 限制范围
